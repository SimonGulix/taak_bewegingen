...
% program data
fig_kin_4bar = 0;           % draw figures if 1
fig_kin_check = 0; 
fig_dyn_4bar = 0;       
fig_dyn_check = 0;
fig_dyn_check_shaking=0;

% kinematic parameters (link lengths) in meters
r11 = 2*22*(10)^(-2);
r12 = 2*sqrt(185)*(10)^(-2);
r13 = 2*sqrt(185)*(10)^(-2);
r2 = 2*4*(10)^(-2);
...
density=8000; %kg/m3
Asurf=(pi/4)*0.03^2; %0.03 m diameter
...
phi2= pi*(1-cos((t/2))); %%%( Harmonic H5-lecture 7, slide 45)
dphi2=(pi/2)*sin(t/2);
ddphi2 =(pi/4)*cos(t/2);
